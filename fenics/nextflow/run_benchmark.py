...
def create_shared_conda_env_dir(benchmark_dir: Path) -> Path:
    """Create and return the shared Nextflow conda environment cache directory."""
...
def build_nextflow_command(parameter_file: Path) -> list[str]:
    """Build the Nextflow command for one configuration."""
    return [
        "nextflow",
        "run",
        "main.nf",
...
def run_nextflow_workflow(
    ...
    """Run the Nextflow workflow and package its RO-Crate output as a zip."""
    cmd = build_nextflow_command(parameter_file)
    env = {**os.environ, "NXF_CONDA_CACHEDIR": str(shared_env_dir)}

    subprocess.run(cmd, check=True, cwd=output_dir, env=env)

    crate = ROCrate(output_dir)
    crate.write_zip(output_dir / f"{TOOL_NAME}-{configuration}.zip")
